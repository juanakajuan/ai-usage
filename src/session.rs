...
/// Maps Codex records onto bundled pricing model identifiers.
fn codex_model_with_configuration(model: String, _reasoning_effort: Option<&str>) -> String {
    if model == "gpt-5.5" {
...
mod tests {
    ...
    fn parses_valid_session_file_without_prompt_content() {
        ...
        assert_eq!(codex_session.compact_model_label(), "gpt-5.5-fast · medium");
    ...
    #[test]
    fn codex_sessions_assume_fast_model_display_and_pricing_identifier() {
        let temporary_directory = tempfile::tempdir().expect("temporary directory");
        let session_file_path = temporary_directory.path().join("session.jsonl");
        write_session_file(
            &session_file_path,
        ...
        let codex_session = &parsed_usage.codex_sessions[0];
        assert_eq!(codex_session.model, "gpt-5.5-fast");
        assert_eq!(codex_session.reasoning_effort, None);
        assert_eq!(codex_session.compact_model_label(), "gpt-5.5-fast");
    }
